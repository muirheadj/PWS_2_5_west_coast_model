shinyServer(function(input, output, session) {
  ...
  output$port_pop_Plot <- renderPlotly({
    portReaderData <- reactiveFileReader(10000, session,
      path(root_dir(), "logs", paste0("parameter00", input$param,
         "_ports_pop_trace.log")),
      read_log)

    port_pop_data <- setNames(portReaderData(), c("level", "timestamp",
      "parameter", "measurement", "iteration", "larva", "cyprid", "juvenile",
      "adult"))
      ...
      gather(lifestage, population, -parameter, -iteration, factor_key = TRUE)
 	  ...
 	  q1 <- plot_ly(port_pop_melt, x = ~iteration, y = ~population,
 	    color = ~lifestage)
 	  add_paths(q1)
 	})
  ...
  output$port_n_Plot <- renderPlotly({
    ...
    port_pop_n_data <- setNames(port_n_ReaderData(), c("level", "timestamp",
      "parameter", "measurement", "iteration", "larva", "cyprid","juvenile",
      "adult"))
